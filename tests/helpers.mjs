...
export const SCRIPTS_DIR = fileURLToPath(new URL("../plugins/grok/scripts/", import.meta.url));
...
export function initGitRepo(prefix = "grok-plugin-repo-") {
 ...
 */
export function initEmptyGitRepo(prefix = "grok-plugin-empty-repo-") {
  const dir = mkTmpDir(prefix);
  git(dir, ["init", "-q"]);
  git(dir, ["config", "user.email", "test@example.com"]);
  git(dir, ["config", "user.name", "Test"]);
 ...
 * scripts don't need a live /grok:setup run.
 *
...
export function isolatedEnv({ withPluginConfig = true } = {}) {
  const home = mkTmpDir("grok-plugin-home-");
  const configDir = path.join(home, ".config", "grok-plugin-cc");
  const env = {
    ...process.env,
    HOME: home,
