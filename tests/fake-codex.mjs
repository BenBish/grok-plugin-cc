...
// Simple cross-process invocation counter, backed by a file the test
// supplies via FAKE_CODEX_COUNTER_FILE — each fake-codex invocation is a
// separate process, so in-memory state can't survive across calls, unlike
// a real long-lived rate-limited backend which local-model-plugin-cc never
// needed to simulate.
...
function main() {
  ...
  switch (mode) {
    ...
    case "rate-limited-always": {
      emit({ type: "item.completed", item: { id: "item_0", type: "error", message: "429 Too Many Requests" } });
      process.exit(1);
      break;
    }
    case "rate-limited-then-success": {
      const counterFile = process.env.FAKE_CODEX_COUNTER_FILE;
      const failuresBeforeSuccess = Number(process.env.FAKE_CODEX_RATE_LIMIT_FAILURES ?? "1");
