---
description: Run a read-only code review through Grok 4.5 (uncommitted changes by default, or --base <ref>).
argument-hint: "[--base <ref>] [--wait|--background]"
---

Run a Grok 4.5 review of the current work. This command never mutates
...
`/grok:status` or `/grok:result` for progress/results.

If the command reports "No Grok configuration found," tell the user
to run `/grok:setup` first — do not attempt to guess a configuration.
