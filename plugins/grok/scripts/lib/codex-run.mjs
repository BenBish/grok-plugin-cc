...
export function checkCodexOnPath() {
  ...
  if (result.error && /** @type {NodeJS.ErrnoException} */ (result.error).code === "ENOENT") {
    throw new CodexNotFoundError(
      "codex CLI not found on PATH. Install it (see https://developers.openai.com/codex/cli) and re-run /grok:setup.",
    );
  }
  if (result.status !== 0) {
    throw new CodexNotFoundError(
...
function tmpOutputFile() {
  return path.join(
    os.tmpdir(),
    `grok-plugin-cc-${process.pid}-${crypto.randomBytes(4).toString("hex")}.txt`,
  );
}
