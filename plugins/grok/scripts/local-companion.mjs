#!/usr/bin/env node
// Broker invoked by the /grok:review, /grok:adversarial-review, and
// /grok:rescue commands (the latter via the grok-rescue subagent). Usage:
//
...
import { withRateLimitRetry } from "./lib/retry.mjs";
...
// Overridable only so tests can exercise the rate-limit retry path without
// real multi-second waits; production always uses the 2s default.
const RETRY_BASE_DELAY_MS = process.env.GROK_RETRY_BASE_DELAY_MS
...
function requireConfig() {
  ...
  if (!config) fail("No Grok configuration found. Run /grok:setup first.");
  return config;
}
...
async function runReviewJob({ job, repoRoot, repoIdValue, kind, args, config }) {
  ...
  // pathological pair of near-timeout calls could take up to 2x the stated
  // timeout for one review.
...
async function main() {
  ...
  if (args.background) {
    ...
    console.log(`Started background job ${job.id}. Check with /grok:status or /grok:result.`);
    return;
  }
